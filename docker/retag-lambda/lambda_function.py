...
def retag(repo: str, source: str, destination: str) -> str:
    ...
    return subprocess.run(
        ["./docker-retag", f"straiproject/{repo}:{source}", destination],
...
def lambda_handler(event, context):
    ...
    for repo in ["strai", "strai-ml"]:
        results = []
        # For example tag strai:1.X-py3.7-cu112 to strai:latest-py3.7-cu112
        for pyversion in python_versions:
            ...
            # Tag strai:1.X-py3.7 to strai:latest-py3.7
            results.append(retag(repo, source_tag, destination_tag))
            # Tag strai:1.X-py3.7-cpu to strai:latest-py3.7-cpu
            results.append(retag(repo, source_tag + "-cpu", destination_tag + "-cpu"))
            # Tag strai:1.X-py3.7-gpu to strai:latest-py3.7-gpu
