...
#include "strai/common/strai_config.h"
#include "strai/util/process.h"
#include "strai/util/util.h"
#include "src/strai/protobuf/gcs.pb.h"
...
void ProcessHelper::StartStraiNode(const std::string node_id_address,
                                 const int port,
                                 const std::string redis_password,
                                 const std::vector<std::string> &head_args) {
  std::vector<std::string> cmdargs({"strai",
                                    "start",
                                    "--head",
                                    "--port",
                                    std::to_string(port),
  ...
  STRAI_LOG(INFO) << CreateCommandLine(cmdargs);
  auto spawn_result = Process::Spawn(cmdargs, true);
  STRAI_CHECK(!spawn_result.second);
  spawn_result.first.Wait();
  return;
}

void ProcessHelper::StopStraiNode() {
  std::vector<std::string> cmdargs({"strai", "stop"});
  STRAI_LOG(INFO) << CreateCommandLine(cmdargs);
  auto spawn_result = Process::Spawn(cmdargs, true);
  STRAI_CHECK(!spawn_result.second);
  spawn_result.first.Wait();
...
std::unique_ptr<strai::gcs::GlobalStateAccessor> ProcessHelper::CreateGlobalStateAccessor(
    const std::string &gcs_ip, int gcs_port) {
  strai::gcs::GcsClientOptions client_options(gcs_ip,
                                            gcs_port,
                                            strai::ClusterID::Nil(),
                                            /*allow_cluster_id_nil=*/true,
                                            /*fetch_cluster_id_if_nil=*/false);
  auto global_state_accessor =
      std::make_unique<strai::gcs::GlobalStateAccessor>(client_options);
  STRAI_CHECK(global_state_accessor->Connect()) << "Failed to connect to GCS.";
  return global_state_accessor;
}

void ProcessHelper::StraiStart(CoreWorkerOptions::TaskExecutionCallback callback) {
  ...
  std::unique_ptr<strai::gcs::GlobalStateAccessor> global_state_accessor =
  ...
  if (ConfigInternal::Instance().worker_type == WorkerType::DRIVER) {
    ...
    STRAI_CHECK_OK(status);
    strai::rpc::GcsNodeInfo node_info;
    node_info.ParseFromString(node_to_connect);
    ConfigInternal::Instance().strailet_socket_name = node_info.strailet_socket_name();
  ...
  STRAI_CHECK(!ConfigInternal::Instance().strailet_socket_name.empty());
  STRAI_CHECK(!ConfigInternal::Instance().plasma_store_socket_name.empty());
  STRAI_CHECK(ConfigInternal::Instance().node_manager_port > 0);
  ...
  options.strailet_socket = ConfigInternal::Instance().strailet_socket_name;
  ...
  options.strailet_ip_address = node_ip;
  ...
  job_config.set_strai_namespace(ConfigInternal::Instance().strai_namespace);
  ...
  STRAI_CHECK(job_config.SerializeToString(&serialized_job_config));
  options.serialized_job_config = serialized_job_config;
  CoreWorkerProcess::Initialize(options);
}

void ProcessHelper::StraiStop() {
