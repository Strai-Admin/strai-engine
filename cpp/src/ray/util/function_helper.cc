...
#include "strai/common/constants.h"
...
void FunctionHelper::LoadDll(const std::filesystem::path &lib_path) {
  STRAI_LOG(INFO) << "Start loading the library " << lib_path << ".";
  ...
  STRAI_CHECK(std::filesystem::exists(lib_path))
  ...
  try {
    ...
    STRAI_LOG(FATAL) << "Failed to load library, lib_path: " << lib_path
  ...
  STRAI_CHECK(libraries_.emplace(lib_path.string(), lib).second);
  ...
  try {
    ...
    if (function_names.empty()) {
      STRAI_LOG(WARNING)
          ...
          << ". If you've already used Strai::Task or Strai::Actor in the library, please "
             "ensure the remote functions have been registered by `STRAI_REMOTE` macro.";
      lib->unload();
      return;
    }
    STRAI_LOG(INFO) << "The library " << lib_path
                  << " is loaded successfully. The remote functions: " << function_names
                  << ".";
    return;
  } catch (boost::system::system_error &e) {
    STRAI_LOG(INFO) << "The library " << lib_path << " isn't integrated with Strai, skip it.";
    lib->unload();
  } catch (std::exception &e) {
    STRAI_LOG(WARNING) << "Failed to get entry function from library: " << lib_path
                     << ", failed reason: " << e.what();
    lib->unload();
  } catch (...) {
    STRAI_LOG(WARNING) << "Failed to get entry function from library: " << lib_path
...
std::string FunctionHelper::LoadAllRemoteFunctions(const std::string lib_path,
  ...
  if (!lib.has(internal_function_name)) {
    STRAI_LOG(WARNING) << "Internal function '" << internal_function_name
                     << "' not found in " << lib_path;
    return "";
  }
  // Both default worker and user dynamic library static link libstrai_api.so which has a
  // singleton class StraiRuntimeHolder, the user dynamic library will get a new un-init
  // instance of StraiRuntimeHolder, so we need to init the StraiRuntimeHolder singleton when
  // loading the user dynamic library to make sure the new instance valid.
  auto init_func =
      boost::dll::import_alias<void(std::shared_ptr<StraiRuntime>)>(lib, "InitStraiRuntime");
  init_func(StraiRuntimeHolder::Instance().Runtime());
...
void FunctionHelper::LoadFunctionsFromPaths(const std::vector<std::string> &paths) {
  ...
  for (auto path : paths) {
    if (std::filesystem::is_directory(path)) {
      ...
      STRAI_LOG(FATAL) << path << " dynamic library not found.";
    }
  }

  STRAI_LOG(INFO) << std::string(kLibraryPathEnvName) << ": " << getLibraryPathEnv();
...
const EntryFuntion &FunctionHelper::GetExecutableFunctions(
  ...
  if (it == remote_funcs_.end()) {
    throw StraiFunctionNotFound("Executable function not found, the function name " +
                              function_name);
  } else {
    return it->second;
  }
...
const EntryFuntion &FunctionHelper::GetExecutableMemberFunctions(
  ...
  if (it == remote_member_funcs_.end()) {
    throw StraiFunctionNotFound("Executable member function not found, the function name " +
                              function_name);
  } else {
    return it->second;
  }
