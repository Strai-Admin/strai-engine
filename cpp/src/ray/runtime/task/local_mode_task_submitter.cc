...
LocalModeTaskSubmitter::LocalModeTaskSubmitter(
    LocalModeStraiRuntime &local_mode_strai_tuntime)
    : local_mode_strai_tuntime_(local_mode_strai_tuntime) {
...
ObjectID LocalModeTaskSubmitter::Submit(InvocationSpec &invocation,
  ...
  builder.SetCommonTaskSpec(task_id,
                            ...
                            rpc::Language::CPP,
                            functionDescriptor,
                            local_mode_strai_tuntime_.GetCurrentJobID(),
                            rpc::JobConfig(),
                            local_mode_strai_tuntime_.GetCurrentTaskId(),
                            0,
                            local_mode_strai_tuntime_.GetCurrentTaskId(),
                            address,
                            1,
  ...
  if (invocation.task_type == TaskType::NORMAL_TASK) {
    ...
    invocation.actor_id = local_mode_strai_tuntime_.GetNextActorID();
    ...
    // NOTE: Strai CPP doesn't support retries and retry_exceptions.
    ...
    throw StraiException("unknown task type");
  }
  ...
  AbstractStraiRuntime *runtime = &local_mode_strai_tuntime_;
...
ActorID LocalModeTaskSubmitter::GetActor(const std::string &actor_name,
                                         const std::string &strai_namespace) const {
  absl::MutexLock lock(&named_actors_mutex_);
...
strai::PlacementGroup LocalModeTaskSubmitter::CreatePlacementGroup(
    const strai::PlacementGroupCreationOptions &create_options) {
  strai::PlacementGroup placement_group{
      PlacementGroupID::Of(local_mode_strai_tuntime_.GetCurrentJobID()).Binary(),
      create_options};
...
std::vector<PlacementGroup> LocalModeTaskSubmitter::GetAllPlacementGroups() {
  throw StraiException("Strai doesn't support placement group operations in local mode.");
}
