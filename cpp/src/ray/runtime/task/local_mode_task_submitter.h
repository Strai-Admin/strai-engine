...
class LocalModeTaskSubmitter : public TaskSubmitter {
 public:
  LocalModeTaskSubmitter(LocalModeStraiRuntime &local_mode_strai_tuntime);
  ...
  ActorID GetActor(const std::string &actor_name, const std::string &strai_namespace) const;
 ...
 private:
  ...
  std::unordered_map<std::string, strai::PlacementGroup> placement_groups_;
};
}  // namespace internal
}  // namespace strai