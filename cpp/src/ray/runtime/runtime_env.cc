...
#include "src/strai/protobuf/runtime_env_common.pb.h"
...
void RuntimeEnv::SetJsonStr(const std::string &name, const std::string &json_str) {
  try {
    ...
    throw strai::internal::StraiRuntimeEnvException("Failed to set the field " + name +
...
std::string RuntimeEnv::GetJsonStr(const std::string &name) const {
  if (!Contains(name)) {
    throw strai::internal::StraiRuntimeEnvException("The field " + name + " not found.");
  }
...
std::string RuntimeEnv::SerializeToRuntimeEnvInfo() const {
  ...
  STRAI_CHECK(google::protobuf::util::MessageToJsonString(runtime_env_info,
...
RuntimeEnv RuntimeEnv::Deserialize(const std::string &serialized_runtime_env) {
  ...
  try {
    ...
    throw strai::internal::StraiRuntimeEnvException("Failed to deserialize runtime env " +
