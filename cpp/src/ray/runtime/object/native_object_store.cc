...
void NativeObjectStore::PutRaw(std::shared_ptr<msgpack::sbuffer> data,
  ...
  auto buffer = std::make_shared<::strai::LocalMemoryBuffer>(
      reinterpret_cast<uint8_t *>(data->data()), data->size(), true);
  auto status = core_worker.Put(
      ::strai::StraiObject(buffer, nullptr, std::vector<rpc::ObjectReference>()),
      {},
      object_id);
  if (!status.ok()) {
    throw StraiException("Put object error");
  }
  return;
}
...
void NativeObjectStore::CheckException(const std::string &meta_str,
  ...
  if (meta_str == std::to_string(strai::rpc::ErrorType::WORKER_DIED)) {
    throw StraiWorkerException(std::move(data_str));
  } else if (meta_str == std::to_string(strai::rpc::ErrorType::ACTOR_DIED)) {
    throw StraiActorException(std::move(data_str));
  } else if (meta_str == std::to_string(strai::rpc::ErrorType::OBJECT_UNRECONSTRUCTABLE) ||
             meta_str == std::to_string(strai::rpc::ErrorType::OBJECT_LOST) ||
             meta_str == std::to_string(strai::rpc::ErrorType::OWNER_DIED) ||
             meta_str == std::to_string(strai::rpc::ErrorType::OBJECT_DELETED)) {
    // TODO: Differentiate object errors.
    throw UnreconstructableException(std::move(data_str));
  } else if (meta_str == std::to_string(strai::rpc::ErrorType::TASK_EXECUTION_EXCEPTION)) {
    throw StraiTaskException(std::move(data_str));
  }
}
...
std::vector<std::shared_ptr<msgpack::sbuffer>> NativeObjectStore::GetRaw(
  ...
  auto &core_worker = CoreWorkerProcess::GetCoreWorker();
  std::vector<std::shared_ptr<::strai::StraiObject>> results;
  ::strai::Status status = core_worker.Get(ids, timeout_ms, results);
  if (!status.ok()) {
    if (status.IsTimedOut()) {
      throw StraiTimeoutException("Get object error:" + status.message());
    }
    throw StraiException("Get object error: " + status.ToString());
  }
  STRAI_CHECK(results.size() == ids.size());
...
std::vector<bool> NativeObjectStore::Wait(const std::vector<ObjectID> &ids,
  ...
  ::strai::Status status = core_worker.Wait(ids, num_objects, timeout_ms, &results, true);
  if (!status.ok()) {
    throw StraiException("Wait object error: " + status.ToString());
  }
  return results;
}
