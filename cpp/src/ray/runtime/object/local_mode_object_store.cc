...
namespace strai {
namespace internal {
LocalModeObjectStore::LocalModeObjectStore(LocalModeStraiRuntime &local_mode_strai_tuntime)
...
void LocalModeObjectStore::PutRaw(std::shared_ptr<msgpack::sbuffer> data,
  ...
  auto status = memory_store_->Put(
      ::strai::StraiObject(buffer, nullptr, std::vector<rpc::ObjectReference>()), object_id);
  if (!status) {
...
std::vector<std::shared_ptr<msgpack::sbuffer>> LocalModeObjectStore::GetRaw(
  ...
  ::strai::Status status = memory_store_->Get(ids,
                                            (int)ids.size(),
                                            timeout_ms,
                                            local_mode_strai_tuntime_.GetWorkerContext(),
                                            false,
                                            &results);
  if (!status.ok()) {
...
std::vector<bool> LocalModeObjectStore::Wait(const std::vector<ObjectID> &ids,
  ...
  ::strai::Status status = memory_store_->Wait(memory_object_ids,
                                             num_objects,
                                             timeout_ms,
                                             local_mode_strai_tuntime_.GetWorkerContext(),
                                             &ready);
