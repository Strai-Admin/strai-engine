...
void Init(strai::StraiConfig &config, int argc, char **argv) {
  if (!IsInitialized()) {
    ...
    auto runtime = internal::AbstractStraiRuntime::DoInit();
    is_init_ = true;
  }
}

void Init(strai::StraiConfig &config) { Init(config, 0, nullptr); }
...
void Shutdown() {
  // TODO(SongGuyang): Clean the strai runtime.
  internal::AbstractStraiRuntime::DoShutdown();
  is_init_ = false;
}

void RunTaskExecutionLoop() { ::strai::core::CoreWorkerProcess::RunTaskExecutionLoop(); }
