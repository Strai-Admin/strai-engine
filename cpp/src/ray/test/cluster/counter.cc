...
Counter::Counter(int init, bool with_exception) {
  ...
  count = init;
  is_restared = strai::WasCurrentActorRestarted();
}
...
bool Counter::CheckRestartInActorTask() { return strai::WasCurrentActorRestarted(); }

strai::ActorHandle<Counter> Counter::CreateChildActor(std::string actor_name) {
  auto child_actor =
      strai::Actor(STRAI_FUNC(Counter::FactoryCreate)).SetName(actor_name).Remote();
  child_actor.Task(&Counter::GetCount).Remote().Get();
  return child_actor;
}

std::string Counter::GetNamespaceInActor() { return strai::GetNamespace(); }

std::string Counter::CreateNestedChildActor(std::string actor_name) {
  child_actor = strai::Actor(STRAI_FUNC(Counter::FactoryCreate)).SetName(actor_name).Remote();
  child_actor.Task(&Counter::GetCount).Remote().Get();
  return "OK";
}

int Counter::Plus1ForActor(strai::ActorHandle<Counter> actor) {
  return *actor.Task(&Counter::Plus1).Remote().Get();
}

int Counter::GetIntByObjectRef(strai::ObjectRef<int> object_ref) {
  return *object_ref.Get();
}

STRAI_REMOTE(STRAI_FUNC(Counter::FactoryCreate),
           Counter::FactoryCreateException,
           STRAI_FUNC(Counter::FactoryCreate, int),
           STRAI_FUNC(Counter::FactoryCreate, int, int),
           &Counter::Plus1,
           &Counter::Add,
           &Counter::Exit,
           &Counter::GetPid,
           ...
           &Counter::EchoStrings,
           &Counter::EchoAnyArstrai);

STRAI_REMOTE(ActorConcurrentCall::FactoryCreate, &ActorConcurrentCall::CountDown);
