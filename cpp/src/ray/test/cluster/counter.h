...
class Counter {
 public:
  ...
  strai::ActorHandle<Counter> CreateChildActor(std::string actor_name);
  std::string CreateNestedChildActor(std::string actor_name);
  int Plus1ForActor(strai::ActorHandle<Counter> actor);
  ...
  std::vector<std::any> EchoAnyArstrai(const std::vector<std::any> &anys) {
    assert(anys.size() == 11);
    // check bool
  ...
  bool Initialized() { return strai::IsInitialized(); }
  ...
  int GetIntByObjectRef(strai::ObjectRef<int> object_ref);

 private:
  int count;
  bool is_restared = false;
  strai::ActorHandle<Counter> child_actor;
};
