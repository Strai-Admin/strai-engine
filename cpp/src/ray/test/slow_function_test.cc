...
STRAI_REMOTE(slow_function);

TEST(StraiSlowFunctionTest, BaseTest) {
  strai::StraiConfig config;
  config.local_mode = true;
  strai::Init(config);
  auto time1 = std::chrono::duration_cast<std::chrono::milliseconds>(
      std::chrono::system_clock::now().time_since_epoch());
  auto r0 = strai::Task(slow_function).Remote(1);
  auto r1 = strai::Task(slow_function).Remote(2);
  auto r2 = strai::Task(slow_function).Remote(3);
  auto r3 = strai::Task(slow_function).Remote(4);
