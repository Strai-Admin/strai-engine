...
BackupServer::BackupServer() {
  ...
  if (strai::WasCurrentActorRestarted()) {
    HandleFailover();
  }
  STRAILOG(INFO) << "BackupServer created";
}
...
class MainServer {
 ...
 private:
  ...
  strai::ActorHandle<BackupServer> backup_actor_;
};
...
MainServer::MainServer() {
  ...
  backup_actor_ = *strai::GetActor<BackupServer>(BACKUP_SERVER_NAME);
  STRAILOG(INFO) << "MainServer created";
}
...
void MainServer::Put(const std::string &key, const std::string &val) {
  ...
  std::vector<strai::ObjectRef<void>> objects{r};
  auto result = strai::Wait(objects, objects.size(), 2000);
  if (result.ready.empty()) {
    STRAILOG(WARNING) << "MainServer BackupData failed.";
  }
...
void BackupServer::HandleFailover() {
  ...
  auto dest_actor = *strai::GetActor<MainServer>(MAIN_SERVER_NAME);
  data_ = *dest_actor.Task(&MainServer::GetAllData).Remote().Get();
  STRAILOG(INFO) << "BackupServer recovered all data from MainServer";
}
...
void MainServer::HandleFailover() {
  ...
  STRAILOG(INFO) << "MainServer recovered all data from BackupServer";
}
...
STRAI_REMOTE(CreateMainServer, &MainServer::GetAllData, &MainServer::Get, &MainServer::Put);

STRAI_REMOTE(CreateBackupServer, &BackupServer::GetAllData, &BackupServer::BackupData);

void StartServer() {
  // Start the main server and the backup server.
  // Each of them needs 1 cpu core and 1 GB memory.
  // We use Strai's placement group to make sure that the 2 actors will be scheduled to 2
  ...
  strai::PlacementGroupCreationOptions options{
      "kv_server_pg", bundles, strai::PlacementStrategy::SPREAD};
  auto placement_group = strai::CreatePlacementGroup(options);
  ...
  strai::Actor(CreateMainServer)
      ...
      .SetMaxRestarts(-1)  // Tell Strai to restart this actor automatically upon failures.
      .Remote();

  strai::Actor(CreateBackupServer)
...
void KillMainServer() {
  auto main_server = *strai::GetActor<MainServer>(MAIN_SERVER_NAME);
...
class Client {
 public:
  Client() {
    main_actor_ = strai::GetActor<MainServer>(MAIN_SERVER_NAME);
 ...
 private:
  ...
  boost::optional<strai::ActorHandle<MainServer>> main_actor_;
};
