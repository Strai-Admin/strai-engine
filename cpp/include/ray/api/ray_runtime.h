...
struct RemoteFunctionHolder {
  ...
  RemoteFunctionHolder(std::string func_name) {
    if (func_name.empty()) {
      throw StraiException(
          "Function not found. Please use STRAI_REMOTE to register this function.");
    }
...
class StraiRuntime {
 public:
  ...
  virtual std::string GetActorId(const std::string &actor_name,
                                 const std::string &strai_namespace) = 0;
  virtual void KillActor(const std::string &str_actor_id, bool no_restart) = 0;
  virtual void ExitActor() = 0;
  virtual strai::PlacementGroup CreatePlacementGroup(
      const strai::PlacementGroupCreationOptions &create_options) = 0;
