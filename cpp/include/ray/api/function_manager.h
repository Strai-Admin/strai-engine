...
PackReturnValue(T result) {
  if constexpr (is_actor_handle_v<T>) {
    auto serialized_actor_handle =
        StraiRuntimeHolder::Instance().Runtime()->SerializeActorHandle(result.ID());
...
/// Manage all strai remote functions, add remote functions by STRAI_REMOTE, get functions by
...
class FunctionManager {
 public:
  ...
  RegisterRemoteFunction(std::string const &name, const Function &f) {
    ...
    if (!pair.second) {
      throw StraiException("Duplicate STRAI_REMOTE function: " + name);
    }
  ...
  static std::string GetClassNameByFuncName(const std::string &func_name) {
    ...
    const std::string &prefix = "STRAI_FUNC(";
    size_t start_pos = 0;
 ...
 private:
  ...
  std::string GetAddress(F f) {
    auto arr = BitCast<std::arstrai<char, sizeof(F)>>(f);
