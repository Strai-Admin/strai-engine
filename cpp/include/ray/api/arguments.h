...
#include <strai/api/xlang_function.h>
 ...
 public:
  ...
  static void WrapArgsImpl(LangType lang_type,
    ...
    if constexpr (is_object_ref_v<OriginArgType>) {
      if (StraiRuntimeHolder::Instance().Runtime()->IsLocalMode()) {
        ...
        throw std::invalid_argument(
            "At present, the Strai C++ API does not support the passing of "
            "`strai::ObjectRef` parameters. Will support later.");
      }
      ...
      if (StraiRuntimeHolder::Instance().Runtime()->IsLocalMode()) {
        auto buffer = StraiRuntimeHolder::Instance().Runtime()->Get(arg.ID());
      ...
      if (lang_type == LangType::CPP) {
        if constexpr (is_actor_handle_v<InputArgTypes>) {
          auto serialized_actor_handle =
              StraiRuntimeHolder::Instance().Runtime()->SerializeActorHandle(arg.ID());
