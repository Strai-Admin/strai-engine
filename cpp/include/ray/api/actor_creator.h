...
#include <strai/api/task_options.h>
...
class ActorCreator {
 public:
  ...
  ActorCreator(StraiRuntime *runtime, RemoteFunctionHolder remote_function_holder)
      : runtime_(runtime), remote_function_holder_(std::move(remote_function_holder)) {}

  template <typename... Args>
  strai::ActorHandle<GetActorType<F>, is_x_lang_v<F>> Remote(Args &&...args);
  ...
  ActorCreator &SetName(std::string name, std::string strai_namespace) {
    create_options_.name = std::move(name);
    create_options_.strai_namespace = std::move(strai_namespace);
    return *this;
  }
  ...
  ActorCreator &SetRuntimeEnv(const strai::RuntimeEnv &runtime_env) {
    create_options_.serialized_runtime_env_info = runtime_env.SerializeToRuntimeEnvInfo();
    return *this;
  }

 private:
  StraiRuntime *runtime_;
