...
enum class StraiLoggerLevel { DEBUG = -1, INFO = 0, WARNING = 1, ERROR = 2, FATAL = 3 };

#define STRAILOG_INTERNAL(level) *CreateStraiLogger(__FILE__, __LINE__, level)
#define STRAILOG(level)                             \
  if (IsLevelEnabled(strai::StraiLoggerLevel::level)) \
  STRAILOG_INTERNAL(strai::StraiLoggerLevel::level)

// To make the logging lib pluggable with other logging libs and make
// the implementation unaware by the user, StraiLog is only a declaration
...
class StraiLogger {
 public:
  ...
  template <typename T>
  StraiLogger &operator<<(const T &t) {
    if (IsEnabled()) {
      Stream() << t;
    }
    return *this;
...
std::unique_ptr<StraiLogger> CreateStraiLogger(const char *file_name,
                                           int line_number,
                                           StraiLoggerLevel severity);
bool IsLevelEnabled(StraiLoggerLevel log_level);
